...
{
	const register uint8_t value = ((mode << CTRL_REG3_PPOD_SHIFT) & CTRL_REG3_PPOD_MASK)
								| ((polarity << CTRL_REG3_IPOL_SHIFT) & CTRL_REG3_IPOL_MASK);
	const register uint8_t mask = ~(CTRL_REG3_IPOL_MASK | CTRL_REG3_PPOD_MASK);
...
void MMA8451Q_ConfigureInterrupt(register mma8451q_interrupt_t irq, register mma8451q_intpin_t pin)
{
	...
	register uint8_t clearMask = I2C_MOD_NO_AND_MASK;
	register uint8_t setMask = (1 << irq);
